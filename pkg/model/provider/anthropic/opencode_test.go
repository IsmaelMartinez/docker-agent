...
// startOpenCodeCapture returns a fake Anthropic endpoint that records the
// x-opencode-session header of every request.
...
// on this path) can be exercised against a local server, plus a snapshot of
// the session header the wrapper itself was handed.
...
func TestOpenCodeSessionHeaderOnCustomProvider(t *testing.T) {
	...
	// A custom provider reaches this client as provider "anthropic" with the
	// OpenCode base URL (see provider.mergeFromProviderConfig).
